...
function TableHeader() {
  return (
    <View style={styles.tableHeaderRow}>
      ...
      <Text style={styles.colVendor}>ชื่อนิติบุคคล / Company Name</Text>
...
function VendorSection({ group, rowOffset }: { group: PndVendorGroup; rowOffset: number }) {
  return (
    <>
...
export function Pnd53PDF({ data }: { data: Pnd53Data }) {
  ...
  const vendorSections = aggregate.groups.map((group, gi) => {
    ...
    return (
      <VendorSection key={group.vendor_id} group={group} rowOffset={offset} />
    );
  });
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.titleBlock}>
          <Text style={styles.titleMain}>ภ.ง.ด.53 — แบบนำส่งภาษีเงินได้หัก ณ ที่จ่าย (นิติบุคคล)</Text>
          <Text style={styles.titleSub}>PND53 — CORPORATE INCOME TAX WITHHOLDING RETURN (JURISTIC PERSONS)</Text>
        ...
        <Text
          ...
          render={({ pageNumber, totalPages }) =>
            `${issuer.name_en}  |  ภ.ง.ด.53 ${filing_no}  |  งวด ${fmtPeriodThai(aggregate.period_code)}  |  หน้า ${pageNumber}/${totalPages}  |  พิมพ์: ${printDate.toISOString().slice(0, 10)}`
          }
          fixed
        />
      </Page>
