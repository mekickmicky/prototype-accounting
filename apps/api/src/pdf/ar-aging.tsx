...
import type { ArAgingResult, AgingCustomerRow } from '../lib/reports/ar-aging';
...
function CustomerRow({ row }: { row: AgingCustomerRow }) {
  return (
    <View style={styles.dataRow}>
      <Text style={styles.colCustomer}>{row.customer_code} {row.customer_name}</Text>
...
export function ArAgingPDF({ result }: { result: ArAgingResult }) {
  const asOfStr =
    ...
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <View style={styles.header}>
          ...
          <Text style={styles.reportTitle}>AR Aging Report / รายงานอายุลูกหนี้</Text>
