...
async function triggerExport(asOf: string, branch: string, format: "csv" | "xlsx" | "pdf") {
  const blob = await apiClient.getBlob(`/api/v1/reports/ap-aging?as_of=${asOf}&branch=${branch}&format=${format}`);
...
function BillRow({ bill }: { bill: AgingBill }) {
  ...
  return (
    <tr className="text-xs text-gray-400 hover:text-gray-200">
      ...
      <td className="py-1">{formatDateBangkok(bill.issue_date)}</td>
      <td className={`py-1 ${overdue ? "text-rose-400" : ""}`}>{formatDateBangkok(bill.due_date)}</td>
...
export default function ApAgingPage() {
  ...
  const run = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await apiClient.get<ApAgingResult>(`/api/v1/reports/ap-aging?as_of=${asOf}&branch=${branch}&format=json`);
      setResult(data);
    } catch (e: unknown) {
