...
// ── Invoice drill-down row ────────────────────────────────────────────────────

function InvoiceRow({ inv }: { inv: AgingInvoice }) {
  ...
  return (
    <tr className="text-xs text-gray-400 hover:text-gray-200">
      ...
      <td className="py-1 text-right">{fmt(inv.total)}</td>
      <td className="py-1 text-right">{fmt(inv.paid_amount)}</td>
      <td className="py-1 text-right font-medium text-white">{fmt(inv.balance)}</td>
    </tr>
  );
}
...
function CustomerRow({ row }: { row: AgingCustomerRow }) {
      ...
      >
        <td className="py-2 px-3 text-sm">
          <span className="inline-flex items-center gap-1">
            ...
            <span className="text-gray-400 mr-1">{row.customer_code}</span>
            <span className="text-white">{row.customer_name}</span>
            <span className="text-gray-600 text-xs ml-1">({row.invoices.length})</span>
          </span>
        </td>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-xs text-gray-500 border-b border-gray-800">
                  <th className="pl-12 py-1 text-left font-normal">Invoice No</th>
...
export default function ArAgingPage() {
  ...
  const [result, setResult] = useState<ArAgingResult | null>(null);
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader title="AR Aging / รายงานอายุลูกหนี้" />
        ...
        <div className="rounded-lg border border-gray-800 overflow-hidden">
          <div className="bg-gray-900 px-4 py-2 flex items-center justify-between">
            <span className="text-xs text-gray-400">
              ...
              · {result.rows.length} customer{result.rows.length !== 1 ? "s" : ""}
            </span>
          </div>
                ...
                {result.rows.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-12 text-center text-gray-500">
                      No open AR as of this date.
                    </td>
                  </tr>
                ) : (
                  result.rows.map(row => <CustomerRow key={row.customer_id} row={row} />)
                )}
              </tbody>
              <tfoot>
