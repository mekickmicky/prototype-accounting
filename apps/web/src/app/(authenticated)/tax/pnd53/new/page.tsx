...
export default function NewPND53Page() {
  ...
  async function handlePreview() {
    ...
    setAggregate(null);
    try {
      const res = await apiClient.post<PndAggregate>("/api/v1/tax-filings/pnd53/preview", { period });
      setAggregate(res);
    } catch (err) {
  ...
  async function handleSaveDraft() {
    ...
    setSaveError(null);
    try {
      const filing = await apiClient.post<TaxFiling>("/api/v1/tax-filings/pnd53", { period });
      router.push(`/tax/pnd53/${filing.id}`);
    } catch (err) {
  ...
  return (
    <div>
      <PageHeader
        title="New ภงด.53 — Withholding Tax (Juristic)"
        description="Generate a new PND53 filing for juristic (corporate) vendors"
        breadcrumbs={[
          { label: "Tax", href: "/tax/pnd53" },
          ...
          >
            ...
            >
              <span style={{ fontSize: 13, fontWeight: 500 }}>
                ผู้รับเงิน · Withholding Records (นิติบุคคล / Juristic)
              </span>
